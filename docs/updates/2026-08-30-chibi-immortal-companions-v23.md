...
- V23 實作提交 `f64cad1` 已推送至 `main` 並由 Render 正式部署。
- `/healthz` 回 200、`status=ok`、`release=chibi-immortal-companions-v23`。
- 正式首頁、封印詳情與收款未開放頁均回 200；首頁載入 V23 CSS、互動腳本與 8 個仙伴操作點。
- 六張正式角色 WebP 全數回 200，回傳大小與本機最佳化檔一致。
- 正式首頁顯示「公開收款仍關閉，不會建立扣款」，結帳連結 0、留言／對話元件 0。
- 正式驗收使用明確機器識別，只讀公開端點；未送出匿名意願、未建立訂單、未登入後台，也未操作客戶資料、Passkey、復原碼、備份、通知或金流憑證。
