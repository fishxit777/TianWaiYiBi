...
- 高／重大事件才送管理員私人 LINE；訊息只含案件編號、事件類型與匿名客戶代碼，不含完整 Email、IP、驗證碼、session 或付款憑證。
- LINE 推播先寫入佇列再送出；送信失敗不回滾付款、權益、裝置或 session，管理員可在後台重試。
...
- 告警 payload 不含完整 Email 或驗證碼；證據鏈逐筆驗證通過。
- 後台可看匿名裝置清單、撤銷裝置、處理風險案件與重試私人 LINE 告警，所有 mutation 仍需管理 session 與 CSRF。
- 自動測試共 51 項通過；Python／JavaScript 語法檢查與 `pip check` 通過。

## 正式公開前 P0

1. 將 `APP_ENV=production`、`COOKIE_SECURE=true`、`ADMIN_IP_ALLOWLIST_REQUIRED=true`。
2. SQLite 換成有獨立最小權限帳號的 PostgreSQL，設定加密備份與還原演練；Render 免費磁碟不可作正式交易證據庫。
...
- 目前沒有管理員 2FA、正式退款撤權、發票或支付對帳排程；SMTP 與綠界仍需用實際正式設定驗收。
- `LINE_ADMIN_USER_ID` 若尚未設定，高／重大事件仍會進佇列與後台，但私人 LINE 狀態會是 `skipped`，不代表事件遺失。
