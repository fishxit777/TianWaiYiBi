...
**Goal:** 不改公開仙策售價，以一筆隔離的 NT$5 信用卡訂單完成正式付款、伺服器回呼、Email 交付、內容開通、退款、撤權與對帳驗收。

**2026-08-25 correction:** 原定 NT$1 已由綠界正式付款頁實證拒絕，原因為低於信用卡建單下限。綠界官方規定信用卡單筆金額最低 NT$5，因此隔離驗證改為 NT$5；公開仙策仍為 NT$199。官方依據：https://support.ecpay.com.tw/11465/
...
1. **採用：後台專用隔離驗證訂單。** 不改 `idea_price` 或任何 `price_override`，因此不會讓訪客看到或買到 NT$5 商品；測完也不需要猜原價再回寫。
2. **不採用：暫時把公開商品改為 NT$5。** 部署與 CDN 時差會產生被外人低價下單的窗口，還可能污染營收與客戶權益。
...
- 驗證訂單固定 `ChoosePayment=Credit`，金額採綠界信用卡官方最低 NT$5，避免其他付款方式的更高門檻。
...
- 本地退款確認只接受 `purpose=verification`、`amount=5`、`status=paid`、正式綠界付款的訂單，且需管理 session、CSRF 與完整訂單編號二次確認。
...
2. 新增管理端建單 API；金額固定 NT$5，Email 只從 Render 私密環境設定取得。
3. 讓驗證訂單可在 `ECPAY_LIVE_CONFIRMED` 尚未開啟時送綠界，但公開商品仍維持關閉。
4. 驗證送往綠界的 `TotalAmount=5`、`ChoosePayment=Credit`、`StoreID=TWYB`，且頁面與 API 不洩露正式憑證。
...
1. 先寫 NT$5 合法回呼、錯誤金額、錯誤 StoreID、重送與模擬付款不得開通測試。
...
3. 部署後確認公開 NT$199 未改、一般結帳仍關閉、後台顯示 NT$5 驗證入口。
4. 在送出正式信用卡付款前取得當下確認；完成 NT$5 實刷、伺服器回呼、Email、一次性開通與內容讀取。
5. 在綠界官方後台查到相同 `TWYB` 驗證訂單與 NT$5 金額；退款送出前再次取得當下確認。
...
- NT$5 實際出現在綠界與本地相同的驗證訂單上，Email、開通碼與內容讀取實測成功。
