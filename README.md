...
- 目錄共有 14 卷封印盲策：第壹卷沿用既有雙層輪胎安全概念，第貳卷至第拾參卷涵蓋地板感測、駕駛風險、弱網生存、智慧農業、受控網路接力、音樂視覺、山區聲學、微瑕疵、密封盒聲學、家電聲紋、活體物流與工具操作安全；第拾肆卷為事件後交通風險的電腦模擬概念方案，V36將額外介紹圖改成單張三區分格3D板：七步流程、狀態轉換與三種待驗證模擬情境，另附對照文字。保留原三張現代概念／模擬規劃圖與六個 Micro-MVP 步驟，原型尚未完成／未驗證。六位原創仙伴依主脈看守各卷。
- 公開收款維持關閉；既有商品資料與舊訂單成交價未調整。使用者核准的逐案一次性暫定價格僅存本機忽略目錄，不對外顯示；後續新想法先完成現代圖文，再另行評價。
...
- V32每次付費圖文存取重新核對session對應的可信裝置；登入碼採原子單次消耗。V36後共80張現版／舊版付費素材位於`private_assets/`（原75張＋V34三張＋V35一張歷史圖＋V36一張現版圖），圖片逐卷驗證paid權益，舊static網址停止提供；歷史圖未刪失。第十四卷頁面仍只呈現四張圖，額外介紹图使用純資料registry，不新增資料庫欄位。公開頁不得放入隱藏的付費圖URL。
- 管理LINE告警使用私人收件人白名單格式、最小化payload、跨worker認領／配額及有上限重試；API接受不代表已讀。到期重試沿用每日摘要工作，沒有另外建立每分鐘排程。詳見V32更新文件。
...
V32一般全套為 `240 passed、5 skipped`；5項PostgreSQL專用測試另於全新loopback PostgreSQL17.11全部通過。測試會先清除繼承的正式服務設定，並且PostgreSQL測試只接受loopback測試實例。
...
- `ECPAY_VERIFICATION_ENABLED`（只在執行隔離 NT$6 正式驗收時暫設為 `true`；不會開放一般商品收款）
- `PAYMENT_VERIFICATION_EMAIL`（只供管理員本人接收 NT$6 驗證開通信）
...
公開收款前仍須完成營運主體與法定揭露、天外專用客服管道、售價、電子發票、付款失敗自助處理及最終銷售文案；綠界技術驗收完成不代表應立即開放收款。
