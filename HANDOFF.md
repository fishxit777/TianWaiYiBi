...
公開留言、匿名留言、評分、買家討論與後台留言審核／回覆入口已全部退役；歷史 `section_messages` 資料表只保留給非破壞性 migration、稽核與備份一致性，不再註冊公開或管理 API。買家只可透過 `/transmission` 私下處理付款、收信、開通、登入與退款問題，不在 LINE 討論或揭露盲策內容。
...
2026-08-27 已完成並正式部署「仙策需求雷達 V1」：六策詳情、閱讀、匿名意願、傳音與交易訊號已接成可稽核漏斗；同一匿名工作階段去重，機器與後台預覽排除，7／30／90 日固定觀察窗與樣本信心門檻均已落地。少於 10 個仙策工作階段不下結論，10～29 個只標探索、不顯示精確排名，至少 30 個才允許方向排序；正式收款關閉時不會用零付款誤判商品失敗。該版正式站曾回報 `release=demand-radar-v1`。
...
2026-08-28 已完成管理摘要訊號品質 V2：歷史 `failed`／`skipped` 通知保留供稽核，但不再冒充今日異常或被批次洪水式重送；每日摘要只把今日真正失敗列入待辦。LINE 官方帳號頻道與管理員私訊收件人分開顯示；綠界正式設定完整但公開收款閘門關閉時標示為正常關閉；流量排除機器及後台預覽並依匿名工作階段去重，熱門仙策未達 10 個工作階段明示樣本不足。
...
- 天外一筆母品牌、天外盲策封印目錄、六脈分類、首卷完整圖文、深淺 Logo、透明 Logo、favicon 與 LINE 圓形頭像。
...
- LINE webhook 簽章、防重送、好友加入、文字指令、已上架封印卷 Flex Carousel 與共用訊息模型的本機模擬器；LINE 卡片同樣不得提前顯示盲策真名。
...
- 後台「傳音對話」工作區只列出卷二六脈，可篩選待審核、私密、已公開與全部訊息，一鍵公開、隱藏或指定回覆；不物理刪除訊息，所有審核／回覆寫入 `audit_logs`。新客戶傳音的 LINE 管理提醒不含客戶身分與正文；客戶回覆提醒信也只要求登入查看，不外送站內內容。
...
- 流量訊號完整性 V2 已完成：歷史事件不刪除，可信基準後才進需求雷達；流量來源改以不重複工作階段計數。可歸因來源只代表搜尋／社群／轉介／LINE／Email 的來源證據，未歸因代表直接或站內流量，兩者都不等於已辨識真人。管理預覽必須經後台身分驗證，固定導回首頁，沒有開放重新導向、IP 指紋或第三方追蹤。
...
- `/`：天外盲策封印目錄與六脈分類
- `/transmission`：私人訂單協助與官方 LINE 加好友 QR；不提供公開留言或概念討論
...
5. 正式綠界與 Brevo 憑證只留在本專案 Render；只有在首卷交付品質、非專屬閱讀授權、退款規則、發票責任與私人客服流程全部定案後，才重新評估開啟公開收款。
