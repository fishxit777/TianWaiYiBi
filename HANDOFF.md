...
- 公開官網字系已依使用者指定的仙俠小說封面題字再校正：正文使用正風毛筆 Regular；標題改用 Bold，並疊白玉／月金雙色、深酒紅墨邊、金色厚度與低位墨影。首頁、六脈商品、結帳、付款、交付、訊息與傳音頁全部套用；後台與本機工具不受影響。
...
- 仙俠封面題字 QA：Regular／Bold 兩套自架 WOFF2 均已載入；H1／H2 computed style 具白玉或月金漸層、約 2px 深酒紅外框與七層右下立體墨影。桌機 1280 分段實看首頁 Hero、主視覺、六脈卡片、入世之法、仙閣心訣、商品、結帳與傳音上下段；手機 430 × 900 再測首頁、商品、結帳與傳音，根頁面 `scrollWidth` 均等於 `clientWidth`，題字未超出安全邊界。
