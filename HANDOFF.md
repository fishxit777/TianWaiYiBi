...
- 公開官網字系已依品牌主視覺中的「天外一筆」金色 Logo 題字重新校正：首頁、六脈商品、結帳、付款、交付、訊息與傳音頁全部使用正風毛筆行書 MasaFont；正文為 Regular，標題為同系 Medium。上一版偏方正楷書的莫大毛筆字體已移除。Logo 題字本身是主視覺內的像素繪字，沒有可輸入的原始字型檔，因此其餘繁中文字採用筆勢最接近且字集完整的自架行書。後台與本機工具不受影響。
...
- Logo 同筆勢全站 QA：兩套自架 WOFF2 均已載入；正文 computed font 為 `Tianwai Masa`、標題為 `Tianwai Masa Display`。桌機 1280 實測首頁、六脈、結帳、傳音與無效訂單頁；手機 430 × 900 再測首頁、六脈、結帳與傳音頁，根頁面 `scrollWidth` 均等於 `clientWidth`，無水平捲動，標題未超出安全邊界。
